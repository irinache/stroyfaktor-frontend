  ...
  <div class="cust-select" :tabindex="tabindex" @blur="open = false" >
    ...
    <div class="items" :class="{selectHide: !open,  select_border: bordered}">
      <div
        ...
        :key="i"
        @click="selected=option; open=false; $root.$emit(select_name, option)"
      >{{ option }}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      selected: this.options.length > 0 ? this.placeholder : null,
      open: false,
    };
  },
  mounted() {
    this.$root.$emit(this.select_name, this.selected);
  }
};
</script>
